...
// Import custom scalars (if any are used in models)
...
async function generateSchema(): Promise<void> {
  ...
  const schema = await gqlSchemaFactory.create(
    [AppResolver, PlanResolver],
    [] // Add scalars here if needed: [JsonObjectScalar]
  )
...
`
  ...
  await app.close()
  console.info('✅ GraphQL schema generated (plan-graphql-ms)')
}
